...
const DEFAULT_ALL_USERS_URL = 'http://localhost:8080/auth/api/auth/getAllusers'
...
export async function fetchProfile() {
  ...
  try {
    ...
    const url = process.env.NEXT_PUBLIC_GET_ALL || DEFAULT_PROFILE_URL
...
export async function fetchAllusers(){
  ...
  if (!token) throw new APIError('Not authenticated', 401, null)
    ...
    const url = process.env.NEXT_PUBLIC_ALL_USERS_URL || DEFAULT_ALL_USERS_URL
    const res = await fetch(url, {
      method: 'GET',
      headers: {
        Accept: 'application/json',
    ...
    if (!res.ok) {
      ...
      const message = (data && data.message) || (rawText && String(rawText).slice(0, 512)) || `Profile fetch failed (${res.status})`
      const responsePayload = data || (rawText ? { text: rawText } : null)
      if (typeof console !== 'undefined' && console.error) console.error('fetchAllUsers error', { status: res.status, response: responsePayload })
      throw new APIError(message, res.status, responsePayload)
    }
    ...
    if (err instanceof APIError) throw err
    if (typeof console !== 'undefined' && console.error) console.error('fetchAllUsers unexpected error', err)
    throw new APIError(err && err.message ? err.message : 'Network error', 0, null)
  }

}

export default { fetchProfile, fetchAllusers }
